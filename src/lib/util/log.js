...
export const CALL_POINT = 1;
// the call stack of console.log
export const CALL_STACK = 2;
...
Object.defineProperty(winObj, "__AT_STACK__", {
  get: function () {
    ...
    for (var i = from; i < errLines.length; i++) {
       ...
       */
      if (
        errLines[i].indexOf("Module._compile") !== -1 ||
    ...
    // original_console_log(errLineArr);
    if (_logType === CALL_POINT) {
      // return errLineArr.slice(0, 1);
      return errLineArr
        .slice(3, 4)
        .map((v) => v.replace("http://localhost:3000", ""));
    } else {
...
console.log = function (...param) {
  ...
  original_console_log.apply(null, param);
};
